...
class FilmDetail extends React.Component {
    ...
    constructor(props) {
        ...
        this._markedFilm = this._markedFilm.bind(this)
    ...
    componentDidMount() {
        ...
        const markedFilmIndex = this.props.markedFilms.findIndex(item => item.id === this.props.navigation.state.params.idFilm)
        ...
        if (markedFilmIndex !== -1) { // Film déjà dans nos films vus, on a déjà son détail
            // Pas besoin d'appeler l'API ici, on ajoute le détail stocké dans notre state global au state de notre component
            this.setState({
                film: this.props.markedFilms[markedFilmIndex]
            }, () => { this._updateNavigationParams() })
            return
        }
    ...
    _markedFilm() {
        const action = { type: "MARKED_FILM", value: this.state.film }
        this.props.dispatch(action)
    }
    ...
    _displayFilmVuActionButton() {
        const { film } = this.state
        var textButton = "Marquer comme vu"

        if (this.props.markedFilms.findIndex(item => item.id === this.props.navigation.state.params.idFilm) !== -1) {
            textButton = "Non vu" // Si le film est dans les films vus, on affiche "Non vu"
        }

        if (film != undefined) { // Lorsque le film est chargé
            return (
                <TouchableOpacity
                    style={styles.film_vu_button}>
                    <Button
                        title={textButton}
                        onPress={() => this._markedFilm()}
                    />
                </TouchableOpacity>
            )
        }
    ...
    render() {
        return (
            <View style={styles.main_container}>
                ...
                {this._displayFilmVuActionButton()}
            </View>
        )
    }
}
